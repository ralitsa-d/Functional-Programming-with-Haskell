...
splitAtBlockSize :: [Char] -> Int -> Int -> [Char] -> Char -> ([Char] -> Char -> [[Char]])
splitAtBlockSize (x:message) blockSize count str result
 {-|message==[] = result  трябва да мине цикъла още веднъж, за да обработи и последното x-}
 |count/=blockSize = splitAtBlockSize message blockSize count+1 (str++x) result
 |count == blockSize = splitAtBlockSize message blockSize 0 [] (result++str)
